...
def load_config():
    ...
    config_file = os.path.join(PROJECT_ROOT, 'Scripts', 'config.ini')
    ...
    if os.path.exists(config_file):
        ...
        for key in ['data_dir', 'output_dir', 'graphics_dir']:
            if key in config['DEFAULT']:
                config['DEFAULT'][key] = os.path.join(PROJECT_ROOT, config['DEFAULT'][key])
    else:
        config['DEFAULT'] = {
            'data_dir': os.path.join(PROJECT_ROOT, 'Data'),
            'output_dir': os.path.join(PROJECT_ROOT, 'Output'),
            'graphics_dir': os.path.join(PROJECT_ROOT, 'Output'),
            'font': 'Arial',
            'font_size': '20',
            'theme': 'light'
        }
...
def plot_survival_by_age(figure, canvas):
    ...
    survival_palette = {0: 'pink', 1: 'palegreen'}
...
def train_model():
    ...
    try:
        ...
        model = Pipeline([
            ...
            ('classifier', RandomForestClassifier(n_estimators=100, random_state=42)) #обучаем RandomForest, фиксируем случайное число 42 для воспроизводимости
        ])
